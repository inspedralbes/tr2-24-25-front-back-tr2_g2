<template>
    <div>
      <div v-for="post in posts" :key="post.id" class="max-w-3xl mx-auto bg-white shadow-lg rounded-lg overflow-hidden mt-6">
        <header class="flex items-center p-4 border-b">
          <img :src="authorAvatar" alt="Avatar" class="w-12 h-12 rounded-full mr-4" />
          <div>
            <h2 class="font-bold text-lg">{{ authorName }}</h2>
            <p class="text-gray-500 text-sm">{{ authorHandle }} · fa {{ timeSince(post.created_at) }}</p>
          </div>
        </header>
        ...
        <main class="p-4 space-y-4">
          ...
          <div v-if="exampleImage" class="rounded-lg overflow-hidden">
            <img :src="exampleImage" alt="Post Image" class="w-full" />
          </div>
        </main>
  ...
  const exampleImage = 'https://d30gl8nkrjm6kp.cloudfront.net/wp/2020/05/Isabel-Diaz-Ayuso-Mariano-Rajoy_1355274498_15007339_660x641.jpg';
